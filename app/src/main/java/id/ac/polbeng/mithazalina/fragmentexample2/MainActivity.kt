package id.ac.polbeng.mithazalina.fragmentexample2

import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import id.ac.polbeng.mithazalina.fragmentexample2.databinding.ActivityMainBinding
...
class MainActivity : AppCompatActivity(), Coordinator {
    ...
    if (savedInstanceState == null) {
        ...
        fragBookTitle = supportFragmentManager.findFragmentByTag(BOOK_TITLE_TAG) as BookTitle
        fragBookDescription = supportFragmentManager.findFragmentByTag(BOOK_DESCRIPTION_TAG) as BookDescription
    }
    ...
    if (!fragBookTitle.isInLayout)
    {
        supportFragmentManager
            .beginTransaction()
            .replace(
                binding.fragtop.id, fragBookTitle,
                BOOK_TITLE_TAG
            )
            .replace(
                binding.fragbottom.id, fragBookDescription,
